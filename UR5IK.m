...
function t=UR5IK(T)
    ...
    % Calculating t1
    p05=T*[0, 0, -d6, 1]'  - [0, 0, 0, 1]';
    aL1=atan2(p05(2),p05(1));
    aL2=acos(d4/sqrt(p05(2)^2 + p05(1)^2));
    t(1,1:4)=pi/2+aL1+aL2;
    t(1,5:8)=pi/2+aL1-aL2;
    t = real(t);
    % Calculate t5
   cols = [1, 5];
    for i=1:length(cols)
        c = cols(i);
        T16 = DHtrans(a1, alpha1, d1, t(1,c))\T;
        p16z = T16(3,4);
        t5=acos((p16z-d4)/d6);
        t(5,c:c+1)=t5;
        t(5,c+2:c+3)=-t5;
    ...
    for i=1:length(cols)
        c = cols(i);
        T01 = DHtrans(a1, alpha1, d1, t(1,c));
        T61 = T\T01;
        T61zy = T61(2, 3);
        T61zx = T61(1, 3);
        t5 = t(5, c);
    ...
    for i=1:length(cols)
        c = cols(i);
        T10 = inv(DHtrans(a1, alpha1, d1, t(1,c)));
        T65 = inv(DHtrans(a6, alpha6, d6, t(6,c)));
        T54 = inv(DHtrans(a5, alpha5, d5, t(5,c)));
        T14 = T10*T*T65*T54;
    ...
    for i=1:length(cols)
        ...
        T32=inv(DHtrans(a3, alpha3, d3, t(3,c)));
        T21=inv(DHtrans(a2, alpha2, d2, t(2,c)));
        T34=T32 * T21 * T14;
